...
static void SwitchInit() { ParsingSwitch = SemanticSwitch = ICSwitch = true; }
  ...
  for (i = 1; i < argc; ++i) {
    ...
    if (ParsingSwitch) {
      ...
      if (SemanticSwitch)
        ...
        printf(
            "\033[31mSemantic analysis %s over(with semantic error).\033[0m\n",
            argv[i]);
    }
    ...
    if (SemanticSwitch) {
      ...
      if (ICSwitch)
        printf("\033[32mIntermediate code generation of %s over(with no "
               "error).\033[0m\n",
               argv[i]);
      else
        printf("\033[31mIntermediate code generation of %s over(with "
               "error).\033[0m\n",
               argv[i]);
    }
