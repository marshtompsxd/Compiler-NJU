  ...
  for (former = FL; former->tail != NULL; former = former->tail) {
    ...
    if(TP->structure.structname != NULL)
    {
        if(strcmp(former->name, TP->structure.structname) == 0)
        {
            ...
            printf("\033[31mError type 15 at Line %d: Redefined field
...
// Error Type 3 and 16
void CheckSameVarNameInSymbolTable(char *varname, int lineno,
                                   SymbolTableHead *table, int Name) {
  SymbolTableEntry *SE;
  for (SE = table->head; SE != NULL; SE = SE->tail) {
    if (SE->kind == VAR && strcmp(varname, SE->Variable.VariableName) == 0) {
      SemanticSwitch = false;
      if (Name == VarName) {
        printf("\033[31mError type 3 at Line %d: Redefined variable "
               "\"%s\".\033[0m\n",
               lineno, varname);
        return;
      } else if (Name == StructName) {
        printf("\033[31mError type 16 at Line %d: Duplicated name "
               "\"%s\".\033[0m\n",
               lineno, varname);
        return;
      } else {
...
void CheckSameFunNameInSymbolTable(char *funname, int lineno,
                                   SymbolTableHead *table) {
  SymbolTableEntry *SE;
  for (SE = table->head; SE != NULL; SE = SE->tail) {
    if (SE->kind == FUN && strcmp(funname, SE->Function.FunName) == 0) {
      SemanticSwitch = false;
      printf("\033[31mError type 4 at Line %d: Redefined function "
             "\"%s\".\033[0m\n",
             lineno, funname);
      return;
    }
...
void CheckSameFieldNameInStruct(char *fieldname, int lineno, Type *TP,
                                int Name) {
  assert(TP->kind == STRUCTURE);
...
void CheckSameVarNameInStructTypeTable(char *varname, int lineno,
                                       StructTypeTableHead *table, int Name) {
...
void CheckElemInTable(SymbolTableHead *table) {
  ...
  for (SE = table->head; SE != NULL; SE = SE->tail) {
    if (SE->kind == VAR) {
      printf("VAR : %s at line : %d, type : ", SE->Variable.VariableName,
             SE->lineno);
...
bool IsTypeInt(Type *T) { return T->kind == BASIC && T->basic == int_type; }

bool IsTypeFloat(Type *T) { return T->kind == BASIC && T->basic == float_type; }

bool IsTypeStruct(Type *T) { return T->kind == STRUCTURE; }

bool IsArray(Type *T) { return T->kind == ARRAY; }

char *GenerateTypeString(Type *TP) {
  if (TP->kind == BASIC) {
    if (TP->basic == int_type)
      return "int";
    else
      return "float";
...
bool CheckParamEquivalence(ParamList *PL1, ParamList *PL2) {
  ...
  while (SE1 != NULL && SE2 != NULL) {
    if (!CheckTypeEquivalence(SE1->Variable.VariableType,
                              SE2->Variable.VariableType))
      return false;
    SE1 = SE1->tail;
    SE2 = SE2->tail;
  }
